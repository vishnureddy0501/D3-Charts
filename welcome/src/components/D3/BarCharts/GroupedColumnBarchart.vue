<template>
    <div class="display-graphs">
        <div class="scrollable-chart">
            <div id="grouped-column-bar-chart"></div>
            <div class="labels">
...
export default {
    ...
    methods: {
        createChart() {
            ...
            const data = [
                { year: 2023, total: 100, female: 50, male: 50, prefixSum: [{male: 0, female: 50}] },
                { year: 2024, total: 200, female: 100, male: 100, prefixSum: [{male: 0, female: 100}] },
                { year: 2025, total: 300, female: 150, male: 150, prefixSum: [{male: 0, female: 150}] },
                { year: 2026, total: 300, female: 150, male: 150, prefixSum: [{male: 0, female: 150}] },
            ];
            ...
            const svg = d3.select("#grouped-column-bar-chart")
                .append("svg")
                .attr("width", width)
                .attr("height", height)
                .append("g")
            ...
            svg.selectAll("g.layer")
                ...
                .attr("height", d => container.height - yScale(d.value))
                .attr("fill", d => colorScale(d.key))
                .attr("rx", 5) // Set the x-axis border radius
                .attr("ry", 5); // Set the y-axis border radius

            // Tooltip
            const tooltip = d3.select("#grouped-column-bar-chart")
                .append("div")
                .attr("class", "tooltip")
            ...
            svg.selectAll("total")
                ...
                .enter()
                .append("rect")
                .attr("class", "total-male")
                .attr("x", d => {
                    return xScale(d.year) + subXScale('total');
                })
                .attr("y", d => yScale(d.female + d.male))
                .attr("width", subXScale.bandwidth())
                .attr("height", d => container.height - yScale(d.male))
                .attr("fill", d => {
            ...
            svg.selectAll(".bars")
                .on("mouseover", (event, d) => {
                    // this is a keyword which contains metadata of the element which triggered this action
                    const containerChart = document.querySelector("#grouped-column-bar-chart").getBoundingClientRect();
                    if (d) {
                        svg.selectAll(".bars")
                            .filter(function () {
                        ...
                        tooltip.html(`<div>${d.key}: ${d.value}</div>`)
                            .style("left", `${event.clientX - containerChart.left + 10}px`)
                            .style("top", `${event.clientY - containerChart.top + 10}px`)
                            .style("opacity", 1);
                    }
                })
            ...
            svg.append('text')
                ...
                .attr('transform', `translate(${container.width / 2}, ${container.height + 50})`);
